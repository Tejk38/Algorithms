...
public class singleEInsortedArray {
    static int singleNonDouble(int[] ar){
        int s = 0;
        int e = ar.length-1;
        while(e>s){
            int mid = s +(e-s)/2;
            if((mid%2 == 0 && ar[mid] == ar[mid+1]) || (mid%2 == 1 && ar[mid] == ar[mid-1])){
                s = mid+1;
            }
            else{
                e = mid;
...
//}
    ...
    public static void main(String[] args) {
        int[] ar = {1, 1, 2, 2, 3, 3, 4, 5, 5};
       // System.out.println(singleNonDuplicate(ar));
        System.out.println(singleNonDouble(ar));
    }
}
